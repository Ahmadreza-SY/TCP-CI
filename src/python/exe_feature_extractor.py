...
class ExeFeatureExtractor:
    ...
    def compute_test_case_features(exe_df):
        ...
        tc_avg_duration = (
            exe_df[["entity_id", "duration"]]
            .groupby("entity_id", as_index=False)
            .mean()
        )
        test_cases = pd.merge(test_cases, tc_avg_duration, on=["entity_id"])
        tc_results = (
            exe_df[["entity_id", "test_result"]]
            .groupby(["entity_id", "test_result"], as_index=False)
            .size()
        )
    ...
    def compute_contributors_failure_rate(
        ...
        commit_to_build_result = dict(
            zip(builds_tc_results.commit_hash, builds_tc_results.test_result)
        )
    ...
    def fetch_and_save_execution_history(args):
        if (args.language, args.level) not in ExeFeatureExtractor.SUPPORTED_LANG_LEVELS:
            print(
                f"Test execution history extraction is not yet supported for {args.language} in {args.level} granularity level."
            )
            return
        ...
        command = f"ruby ./src/ruby/exe_feature_extractor.rb {args.project_slug} {log_type} {args.output_path}"
        return_code = subprocess.call(shlex.split(command))
        if return_code != 0:
            print(f"failed ruby test execution history command: {command}")
            return

        exe_df = pd.read_csv(f"{args.output_path}/test_execution_history.csv")
        test_name_to_id = exe_mapper.compute_test_name_to_id(exe_df, args)
        exe_df["entity_id"] = exe_df["test_name"].apply(
        ...
        exe_df["test_result"] = exe_df["test_result"].apply(
            ...
            if result <= TestResult.UNKNOWN_FAILURE.value
            else TestResult.UNKNOWN_FAILURE.value
        )
        exe_df.sort_values(by=["build", "job"], ascending=False, inplace=True)
        exe_df.to_csv(f"{args.output_path}/exe.csv", index=False)
        os.remove(f"{args.output_path}/test_execution_history.csv")

        ExeFeatureExtractor.compute_test_case_features(exe_df).to_csv(
            f"{args.output_path}/test_cases.csv", index=False
        )

        builds_df = pd.read_csv(
            f"{args.output_path}/builds.csv", sep=args.unique_separator
        )
        builds_df.sort_values(by=["id"], ascending=False, inplace=True)
        builds_df.to_csv(
            f"{args.output_path}/builds.csv", index=False, sep=args.unique_separator
        )
        ...
        contributors_df = ExeFeatureExtractor.compute_contributors_failure_rate(
            exe_df, builds_df, commits_df, contributors_df
        )
        contributors_df.to_csv(f"{args.output_path}/contributors.csv", index=False)

        jobs_df = pd.read_csv(f"{args.output_path}/jobs.csv", sep=args.unique_separator)
        jobs_df.sort_values(by=["build_id", "id"], ascending=False, inplace=True)
        jobs_df.to_csv(
            f"{args.output_path}/jobs.csv", index=False, sep=args.unique_separator
        )
        return
...
class JavaFileExeMapper:
    ...
    def compute_test_name_to_id(exe_df, args):
        metadata_df = pd.read_csv(
            f"{args.output_path}/metadata.csv", usecols=["Id", "Package"]
        )
...
class CFileExeMapper:
    ...
    def compute_test_name_to_id(exe_df, args):
        metadata_df = pd.read_csv(
            f"{args.output_path}/metadata.csv", usecols=["Id", "FilePath", "EntityType"]
        )
