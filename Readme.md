...
### Creating Accurate Understand Databases
...
Additionally, for accurate Java projects, Understand mentions importing the complete classpaths used in your project. For more details visit [this page](https://scitools.com/support/creating-accurate-java-projects/).

### Creating Understand Database Using the GUI
The following instructions are for creating an Understand database using the `compile_commands.json` file generated by the cmake command in C/C++ projects. If this approach does not apply to your project, you can checkout Understand's official documents in [this link](https://scitools.com/support/) and find the best method for your project.

- Click on File -> New -> Project from the title bar.

- Add the root directory of your source code and click continue, like this:

![Add directory](docs/gui-wizard-1.png)

- Next, you can see the "Import Settings from Other Tools?" window which might include lots of options to select from (like the image below). If you have followed the last section, you can see the `compile_commands.json` among the options. Tap the Import button on the right side of this option.

![Import settings](docs/gui-wizard-2.png)

- In this step, you can set the path to save the database file after Understand's analysis. You can also view how many source files Understand has detected. Make sure you have checked the "Configure Advanced Settings" checkbox. Finally, click on Create Project.

![Summary](docs/gui-wizard-3.png)

- In the configuration window you can set all the available settings for your project before starting the code analysis. First, click on the CMake item on the left menu and double check the existence of `compile_commands.json`. Second, you need to add the rest of the source files, and for doing that, select the File item on the menu to view the added source files. Then click on the button showed in the following picture and add the root directory of your project's source code. Finally, click on Ok button and wait for the analysis to complete.

![Add Sources](docs/gui-wizard-4.png)

Note that creating an Understand database for Java projects similar to the above instructions, however, you don't need the compile_commands.json file for Java projects and adding the project's root directory is enough.
